...
class BenchmarkSummary:
    ...
    def to_report(self) -> dict[str, Any]:
        ...
        recall = self.true_positives / self.ground_truth_visible if self.ground_truth_visible else 0.0
        return {
            **{k: v for k, v in asdict(self).items() if not k.endswith("_errors_m") and not k.endswith("_errors_px")},
            "fps": fps,
            "avg_ms_per_frame": (self.total_time_s / self.frames * 1000.0) if self.frames else 0.0,
            "precision": precision,
            "recall": recall,
...
def blender_world_to_cv_camera(camera_matrix_world: list[list[float]], point_world: list[float]) -> np.ndarray:
    camera_world = np.asarray(camera_matrix_world, dtype=float)
    world_point = np.asarray([point_world[0], point_world[1], point_world[2], 1.0], dtype=float)
    ...
    return np.asarray([point_blender_camera[0], -point_blender_camera[1], -point_blender_camera[2]], dtype=float)


def iter_frames(data_root: Path, max_frames: int | None = None) -> Iterable[FrameRecord]:
    yielded = 0
            ...
            for tag in frame.get("tags", []):
                ...
                gt_tag = GroundTruthTag(
                    ...
                    center_image_px=np.asarray(
                        [float(tag["center_ndc"][0]) * intr.width, (1.0 - float(tag["center_ndc"][1])) * intr.height],
                        dtype=float,
                    ),
                    position_camera_cv_m=blender_world_to_cv_camera(frame["camera_matrix_world"], tag["center_world"]),
                    corners_world=np.asarray(tag["corners_world"], dtype=float).reshape(4, 3),
                    center_world=np.asarray(tag["center_world"], dtype=float).reshape(3),
                )
...
def match_by_family_id(gt_tags: list[GroundTruthTag], detections: list[TagDetection]) -> tuple[list[tuple[GroundTruthTag, TagDetection]], list[GroundTruthTag], list[TagDetection]]:
    remaining = detections.copy()
    matches = []
    missed = []
    for gt in gt_tags:
        candidates = [d for d in remaining if d.tag_family == gt.tag_family and d.tag_id == gt.tag_id]
        if not candidates:
            missed.append(gt)
            continue
        det = min(candidates, key=lambda d: float(np.linalg.norm(d.center - gt.center_image_px)))
