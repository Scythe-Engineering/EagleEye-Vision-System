...
public class Robot extends TimedRobot {
  ...
  public Robot() {
    ...
    // Same owner thread as dashboards; consume vision and update odometry at 200 Hz.
  ...
  private static Pose2d simulatedTruth(double elapsed) {
    double angle = 0.2 * elapsed;
    return new Pose2d(4 + 2 * Math.sin(angle), 2 + 2 * (1 - Math.cos(angle)), new Rotation2d(angle));
  }
  @Override public void simulationPeriodic() {
    // Sample truth when the camera stamps it, not at the previous odometry callback.
    simulator.update(simulatedTruth(Timer.getFPGATimestamp() - start));
  }
}
