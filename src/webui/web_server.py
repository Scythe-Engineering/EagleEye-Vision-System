...
class EagleEyeInterface:
    ...
    def _get_test_video_directory(self) -> Path:
        ...
        """
        video_dir = Path(src_path) / "utils" / "sim_videos"
    ...
    def _get_pipeline_references_for_bus_id(self, bus_id: str) -> list[str]:
        ...
        Returns:
            list[str]: Sorted pipeline names with device_input operations using
                the provided bus ID.
        """
        config_path = Path(src_path) / "config" / "pipeline_config.json"
    ...
    def _is_request_flag_enabled(self, flag_name: str) -> bool:
        ...
        """
        raw_value: Any = request.args.get(flag_name)
        if raw_value is None:
            raw_value = request.form.get(flag_name)
        if raw_value is None:
    ...
    def _assets_dir(self) -> Path:
        ...
        return Path(current_path) / "assets"
    ...
    def _overwrite_requested(self) -> bool:
        ...
        return str(request.form.get("overwrite", "")).lower() == "true"
    ...
    def upload_field_file(self) -> tuple[dict, int]:
        ...
        try:
            ...
            file = request.files["file"]
            year = self._sanitize_field_year(str(request.form.get("year", "")))
            filename = self._sanitize_asset_filename(file.filename or "")
            apriltag_map_upload = request.files.get("apriltag_map")
