...
class CameraCalibrationMixin:
    ...
    def _charuco_dictionary(self, dictionary_id: int):
        if not hasattr(cv2, "aruco"):
            raise ValueError("OpenCV ArUco support is unavailable; install opencv-contrib-python")
    ...
    def _resize_for_live_view(self, frame: np.ndarray, live_width: int | None, live_height: int | None) -> np.ndarray:
    ...
    def _find_charuco(self, frame: np.ndarray, board: Any):
        ...
        return found, charuco_corners, charuco_ids, refined_corners, refined_ids, gray.shape[::-1]

    def _find_best_charuco(self, frame: np.ndarray, params: tuple[int, int, float, float, int]):
        best = None
        best_count = -1
    ...
    def _draw_detection(self, frame: np.ndarray, params: tuple[int, int, float, float, int], count: int) -> tuple[np.ndarray, bool]:
        output = frame.copy()
        ...
        cv2.putText(output, text, (16, 32), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 255, 0) if found else (0, 0, 255), 2)
        return output, found

    def _calibration_feed_generator(self, camera_bus_id: str, params: tuple[int, int, float, float, int], live_width: int | None, live_height: int | None) -> Generator[bytes, Any, Any]:
    ...
    def calibration_feed(self, camera_bus_id: str) -> Response:
        ...
        return Response(self._calibration_feed_generator(camera_bus_id, params, live_width, live_height), mimetype="multipart/x-mixed-replace; boundary=frame")
    ...
    def capture_calibration_frame(self, camera_bus_id: str) -> tuple[dict, int]:
        ...
        if not found:
            marker_count = 0 if marker_ids is None else len(marker_ids)
            return {"error": f"ChArUco corners were not detected ({marker_count} markers found)"}, 400
        with self._calibration_lock():
            session = self._calibration_sessions().setdefault(str(camera_bus_id), {"frames": []})
            session.update({"squares_x": best_params[0], "squares_y": best_params[1], "square_size": best_params[2], "marker_size": best_params[3], "dictionary_id": best_params[4], "legacy_pattern": best_params[5]})
            session["frames"].append({"frame": frame, "charuco_corners": charuco_corners, "charuco_ids": charuco_ids, "params": best_params})
            count = len(session["frames"])
        return {"success": True, "frame_count": count, "frame_index": count - 1, "squares_x": best_params[0], "squares_y": best_params[1], "recommended_count": 10}, 200

    def get_calibration_frames(self, camera_bus_id: str) -> tuple[dict, int]:
        with self._calibration_lock():
            frames = list(self._calibration_sessions().get(str(camera_bus_id), {}).get("frames", []))
        response_frames = []
        for i, item in enumerate(frames):
            frame = item.get("frame")
            image_size = {"width": int(frame.shape[1]), "height": int(frame.shape[0])} if frame is not None else None
            corners = item.get("charuco_corners")
            if corners is None:
                corners = item.get("corners")
            response_frames.append({"index": i, "image_size": image_size, "corners": corners.reshape(-1, 2).astype(float).tolist() if corners is not None else []})
        return {"frames": response_frames, "frame_count": len(response_frames), "recommended_count": 10}, 200

    def get_calibration_frame_image(self, camera_bus_id: str, frame_index: int) -> Response | tuple[dict, int]:
        try:
            params = self._charuco_params_from_request()
        except ValueError:
            params = (11, 8, 0.015, 0.011, cv2.aruco.DICT_4X4_50)
        with self._calibration_lock():
            frames = self._calibration_sessions().get(str(camera_bus_id), {}).get("frames", [])
    ...
    def delete_calibration_frame(self, camera_bus_id: str, frame_index: int) -> tuple[dict, int]:
        with self._calibration_lock():
            frames = self._calibration_sessions().get(str(camera_bus_id), {}).get("frames", [])
            if frame_index < 0 or frame_index >= len(frames):
                return {"error": "Frame index out of range"}, 404
    ...
    def run_camera_calibration(self, camera_bus_id: str) -> tuple[dict, int]:
        ...
        for item in frames:
            ...
            if isinstance(item, dict) and item.get("params") != active_params:
                continue
            found, charuco_corners, charuco_ids, _, _, size = self._find_charuco(frame, board)
            if found:
        ...
        rms, camera_matrix, dist, _, _ = cv2.aruco.calibrateCameraCharuco(all_corners, all_ids, board, image_size, None, None)
        target_path = config.intrinsics_path or self._default_intrinsics_path(camera_bus_id)
        os.makedirs(os.path.dirname(target_path), exist_ok=True)
        import json

        with open(target_path, "w") as f:
            json.dump({"camera_matrix": camera_matrix.tolist(), "distortion_coefficients": dist.reshape(-1).tolist(), "reprojection_error": float(rms), "image_size": list(image_size), "charuco": {"squares_x": active_params[0], "squares_y": active_params[1], "square_size": active_params[2], "marker_size": active_params[3], "dictionary": "DICT_4X4_50", "dictionary_id": active_params[4], "legacy_pattern": active_params[5]}}, f, indent=4)
        config.intrinsics_path = target_path
        return {"success": True, "frame_count": len(all_corners), "reprojection_error": float(rms), "intrinsics_path": target_path}, 200
