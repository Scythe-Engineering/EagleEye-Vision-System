...
class MX3ModelIO:
    def __init__(self, model_object: MultiStreamAsyncAccl, input_data_shape: tuple[int, int]):
        """
    ...
    def connect_streams(self, stream_count: int) -> None:
        ...
        """
        self.model_object.connect_streams(self.model_input_generator, self.model_output_processor, stream_count)
...
class MX3Accelerator(ComputeDevice):
    ...
    def load_model(self, model_path: str, input_data_shape: tuple[int, int]) -> None:
        ...
        try:
            ...
            self.model_io_objects[model_name] = MX3ModelIO(model_object=self.models[model_name], input_data_shape=input_data_shape)
    ...
    def run(
        ...
        data_array = data_array.reshape(input_data_shape[0], input_data_shape[1], 1, 1)
        data_array = data_array.astype(np.float32)

        return_data = self.model_io_objects[model_name].sequential_run(stream_idx, data_array)

        reshaped_data = return_data.reshape(return_data.shape[1], return_data.shape[2])
