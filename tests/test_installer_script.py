...
def _run_installer_function(script: str) -> subprocess.CompletedProcess[str]:
    """Source install.sh without installing anything and run shell snippets."""
...
def test_render_service_unit_uses_the_current_user_and_path() -> None:
    ...
    assert "eagle" not in unit.replace("EagleEye", "").replace("Eagle", "")
...
def starter_pipeline_fixture() -> list[dict]:
...
def test_starter_pipeline_ports_match_operation_definitions(
    ...
    for operation in starter_pipeline:
        ...
        definition_path = next(path for path in definition_paths if path.exists())
...
def test_starter_pipeline_passes_runtime_port_validation(
    ...
    """The starter is incomplete by parameters only, not structurally invalid."""
...
def test_starter_pipeline_leaves_camera_specific_values_unset(
    ...
    """The installer must not guess camera IDs, calibration, or extrinsics."""
