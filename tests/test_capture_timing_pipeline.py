...
from src.utils.timing import TimedValue, TimingMetadata, is_timed
...
def test_device_input_never_returns_untimed_frame() -> None:
    """Verify device input always returns a timed packet or None."""
    manager = FakeCameraThreadManager(default_frame=dummy_frame())
    manager.add_camera("camera_1")
    manager.register_bus_id("1", "camera_1")
    ...
    assert packet is None or is_timed(packet)
